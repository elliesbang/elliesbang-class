...
export default function VodUsers() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch("/api/admin-users?role=vod", {
        headers: {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((u) =>
      [u.full_name, u.nickname, u.email]
        .filter(Boolean)
  ...
  return (
    <div className="space-y-6">
      {/* 헤더 */}
      <div>
        <h1 className="text-lg md:text-2xl font-bold mb-1">VOD 사용자 관리</h1>
        <p className="text-sm text-gray-500">VOD 로그인한 사용자 목록입니다.</p>
      </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="space-y-2 text-sm">
              <p className="font-semibold text-lg">{getDisplayName(selectedUser)}</p>
